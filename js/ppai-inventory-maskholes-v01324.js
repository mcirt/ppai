...
const REF=window.PPAI_INVENTORY_CLUSTER_REFERENCE_V01324;
...
async function buildTemplateData(){
  ...
  // v0.13.24 keeps the approved silhouette unchanged as a last-resort fallback.
...
function triangleGeometry(x,y,z){
  const d1=Math.hypot(y.cx-x.cx,y.cy-x.cy),d2=Math.hypot(z.cx-y.cx,z.cy-y.cy);
  const ux=x.cx-y.cx,uy=x.cy-y.cy,vx=z.cx-y.cx,vy=z.cy-y.cy;
  const denom=Math.max(1e-6,d1*d2);
  const cosA=clamp((ux*vx+uy*vy)/denom,-1,1);
  const angleY=Math.acos(cosA)*180/Math.PI;
  // Perpendicular distance from Y to X-Z baseline. This captures the shallow ^ apex
  // independent of camera scale and small whole-cluster rotation.
  const bx=z.cx-x.cx,by=z.cy-x.cy,base=Math.max(1e-6,Math.hypot(bx,by));
  const signed=((bx)*(y.cy-x.cy)-(by)*(y.cx-x.cx))/base; // screen coords: negative means Y above X-Z
  const apexHeight=-signed;
  const meanSide=(d1+d2)/2;
  const apexRatio=apexHeight/Math.max(1,meanSide);
  const spacingRatio=Math.max(d1,d2)/Math.max(1,Math.min(d1,d2));
  return {d1,d2,angleY,apexHeight,apexRatio,spacingRatio};
}
function wedgeScoreFromMetrics(g){
  // Canonical snapshots cluster around ~150° at Y and apex height ~0.26 of side length.
  const angleScore=1-clamp(Math.abs(g.angleY-150)/18,0,1);
  const apexScore=1-clamp(Math.abs(g.apexRatio-.26)/.18,0,1);
  const spacingBalance=1-clamp(Math.abs(g.d1-g.d2)/Math.max(1,(g.d1+g.d2)/2),0,1);
  return {angleScore,apexScore,spacingBalance};
}
function chooseTriplet(comps,w,h){
  ...
  for(let i=0;i<comps.length;i++)for(let j=i+1;j<comps.length;j++)for(let k=j+1;k<comps.length;k++){
    ...
    // HARD SIZE GATE: all three physical inventory windows are essentially uniform.
    if(rMin<=0 || rMax/rMin>1.38)continue;
    const rMean=(x.r+y.r+z.r)/3;
    const rSpread=(rMax-rMin)/Math.max(1,rMean);
    const g=triangleGeometry(x,y,z);
    if(g.d1<rMean*.90||g.d2<rMean*.90||g.d1>rMean*3.0||g.d2>rMean*3.0)continue;
    if(g.spacingRatio>1.35)continue;
    // Measured canonical geometry is a shallow ^, not a straight line.
    // Allow camera/perspective variation but reject flat or sharply bent impostors.
    if(g.angleY<136||g.angleY>166)continue;
    if(g.apexRatio<.08||g.apexRatio>.46)continue;
    const ws=wedgeScoreFromMetrics(g);
    ...
    const score=sizeScore*.27+ws.spacingBalance*.22+ws.angleScore*.23+ws.apexScore*.16+centerScore*.08+verticalScore*.04;
    if(!best||score>best.score)best={slots:{X:x,Y:y,Z:z},score,alignScore:(ws.angleScore+ws.apexScore)/2,sizeScore,spacingBalance:ws.spacingBalance,centerScore,rMean,d1:g.d1,d2:g.d2,angleY:g.angleY,apexRatio:g.apexRatio,source:"three-dark"};
  }
  return best;
}
...
function chooseBestPair(comps,w,h){
  ...
  for(let i=0;i<comps.length;i++)for(let j=i+1;j<comps.length;j++){
    ...
    if(rMin<=0 || rMax/rMin>1.25)continue;
    const rMean=(left.r+right.r)/2;
    const dx=right.cx-left.cx,dy=right.cy-left.cy,dist=Math.hypot(dx,dy);
    if(dist<rMean*.95 || dist>rMean*5.2)continue;
    if(Math.abs(dy)>rMean*1.25)continue;
    const stepRatio=dist/Math.max(1,rMean);
    const hypotheses=[];
    if(stepRatio<=2.9){
      // Adjacent-pair hypotheses use the measured shallow-^ geometry.
      // X->Y rises; Y->Z falls. Reflect vertical displacement instead of extending a straight line.
      hypotheses.push({slots:{X:left,Y:right},missing:"Z",pred:{cx:right.cx+dx,cy:right.cy-dy},kind:"XY"});
      hypotheses.push({slots:{Y:left,Z:right},missing:"X",pred:{cx:left.cx-dx,cy:left.cy+dy},kind:"YZ"});
    }else if(stepRatio<=5.2){
      // X/Z anchors: Y sits above their baseline by ~tan(15°) times one horizontal step.
      const midx=(left.cx+right.cx)/2,midy=(left.cy+right.cy)/2;
      const halfDx=Math.abs(right.cx-left.cx)/2;
      hypotheses.push({slots:{X:left,Z:right},missing:"Y",pred:{cx:midx,cy:midy-halfDx*TAN15},kind:"XZ"});
    }
    for(const hyp of hypotheses){
      ...
      const g=triangleGeometry(x,y,z);
      if(g.spacingRatio>1.18||g.angleY<138||g.angleY>164||g.apexRatio<.10||g.apexRatio>.43)continue;
      const ws=wedgeScoreFromMetrics(g);
      ...
      const score=pairSizeScore*.30+ws.angleScore*.24+ws.apexScore*.18+ws.spacingBalance*.16+centerScore*.08+verticalScore*.04;
      if(!best||score>best.score)best={slots,score,alignScore:(ws.angleScore+ws.apexScore)/2,sizeScore:pairSizeScore,spacingBalance:ws.spacingBalance,centerScore,rMean,d1:g.d1,d2:g.d2,angleY:g.angleY,apexRatio:g.apexRatio,source:"two-dark-infer",missing:hyp.missing,pair:[left,right],pairKind:hyp.kind};
    }
  }
  return best;
}
...
function refineInferredWithComponent(pairLock,comps){
  ...
  if(best){
    ...
    const g=triangleGeometry(x,y,z);
    pairLock.d1=g.d1;pairLock.d2=g.d2;pairLock.angleY=g.angleY;pairLock.apexRatio=g.apexRatio;
  }
  return pairLock;
}
function drawMaskDetection(comps,triplet){
  ...
  if(!triplet)return;
  const colors={X:"#35e5ff",Y:"#ffd84a",Z:"#56e36d"};
  const X=triplet.slots.X,Y=triplet.slots.Y,Z=triplet.slots.Z;
  // v0.13.24: explicitly draw the measured shallow-^ center geometry.
  ctx.save();ctx.strokeStyle="#ff3b30";ctx.lineWidth=Math.max(3,w/220);ctx.beginPath();ctx.moveTo(X.cx,X.cy);ctx.lineTo(Y.cx,Y.cy);ctx.lineTo(Z.cx,Z.cy);ctx.stroke();ctx.restore();
...
async function analyze(){
  ...
  try{
    ...
    if(triplet && triplet.score>=.66){
      ...
      candidateInfo.innerHTML=`<strong>3-CIRCLE DARK-WINDOW LOCK: ${pct(triplet.score)}</strong><br>∠XYZ ${triplet.angleY.toFixed(1)}° · apex ${(triplet.apexRatio*100).toFixed(0)}% · size match ${pct(triplet.sizeScore)} · spacing ${pct(triplet.spacingBalance)} · radius ratio ${sizeRatio.toFixed(2)}×.`;
      if(debugEl)debugEl.textContent=`Three-window path used. Enclosed dark components: ${comps.length}. X/Y/Z side lengths ${triplet.d1.toFixed(1)} / ${triplet.d2.toFixed(1)} px; ∠XYZ ${triplet.angleY.toFixed(1)}°; apex ratio ${(triplet.apexRatio*100).toFixed(0)}%; average radius ${triplet.rMean.toFixed(1)} px. Hard rules: uniform radii ≤1.38×, shallow-∧ angle 136–166°, apex ratio 8–46%. Silhouette fallback was not needed.`;
    ...
    if(pairLock && pairLock.score>=.64){
      ...
      candidateInfo.innerHTML=`<strong>2-CIRCLE + INFERRED ${pairLock.missing} LOCK: ${pct(pairLock.score)}</strong><br>`+
        `two anchors passed ≤1.25× radius agreement · ${pairLock.missing} ${inferredSource==="refined-dark"?"was locally refined":"was predicted from the measured shallow-∧ geometry"} · sides ${pairLock.d1.toFixed(1)} / ${pairLock.d2.toFixed(1)} px · ∠XYZ ${pairLock.angleY.toFixed(1)}°.`;
      if(debugEl)debugEl.textContent=`Two-window geometry path used. Missing ${pairLock.missing} was ${inferredSource==="refined-dark"?"refined from a nearby same-size dark component":"inferred from the canonical shallow-∧ triangle (Y above X/Z)"}. A third candidate cannot be used unless its radius agrees within 1.30× of the trusted pair. Silhouette fallback was not needed.`;
