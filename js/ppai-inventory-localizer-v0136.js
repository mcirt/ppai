...
const REF=window.PPAI_INVENTORY_REFERENCE_V0136;
...
function markCvReady(){
  ...
  if(cvReady){
    opencvStatus.textContent="OpenCV ready — v0.13.6 geometry-first localizer available.";
...
function extractDescriptor(canvas,slot){
  ...
  for(let y=0;y<size;y++)for(let x=0;x<size;x++){
    ...
    const gi=gy*grid+gx;
    lum[gi]+=L;count[gi]+=1;visiblePixels++;
  }
...
async function ensureEmojiReferences(){
  ...
  for(let id=1;id<=18;id++){
    ...
    const img=await loadImage(`images/${emoji}.png?v=0136`);
    const sw=img.naturalWidth||img.width,sh=img.naturalHeight||img.height;
...
function classifyCrop(canvas,slot,refs){
  ...
  work.width=96;work.height=96;
  work.getContext("2d",{willReadFrequently:true}).drawImage(canvas,0,0,96,96);
  const desc=extractDescriptor(work,slot);
  ...
  const ranked=candidates.map(r=>{
    ...
    return {emoji:r.emoji,id:r.id,score:color*0.16+shape*0.56+structure*0.28,color,shape,structure};
  }).sort((a,b)=>b.score-a.score);
  const best=ranked[0],second=ranked[1];
  const margin=Math.max(0,best.score-second.score);
  const familyMargin=Math.max(0,families[0].score-(families[1]?.score||0));
  const confidence=clamp(0.18+margin*4.5+familyMargin*1.6+Math.max(0,best.shape-0.58)*0.55,0,1);
  return {emoji:best.emoji,id:best.id,family:family.name,confidence,margin,secondEmoji:second.emoji};
}
...
async function runRecognition(){
  ...
  for(const slot of ["X","Y","Z"]){
    ...
    readings[slot]=classifyCrop(c,slot,refs[slot]);
    const r=readings[slot];
    const el=document.getElementById(`read${slot}`);
    const strong=r.confidence>=0.68&&r.margin>=0.035;
    el.className=`reading ${strong?"high":"low"}`;
    el.innerHTML=`${r.emoji.replace("emoji","E")} · ${r.family} · ${pct(r.confidence)}<br><span style="font-weight:600">runner-up ${r.secondEmoji.replace("emoji","E")} · Δ ${r.margin.toFixed(3)}</span>`;
  }
  const weak=["X","Y","Z"].filter(s=>readings[s].confidence<0.68||readings[s].margin<0.035);
  document.getElementById("recognitionSummary").textContent=
    weak.length
      ? `Recognition complete. ${weak.length} slot(s) are low-confidence; review the crop and result.`
...
async function buildTemplateEdgePoints(){
  ...
  const [img,maskImg]=await Promise.all([
    loadImage("images/inventory_reference_v0136.png?v=0136"),
    loadImage("images/inventory_reference_mask_v0136.png?v=0136")
  ]);
...
async function analyze(){
  ...
  try{
    ...
    const yCirc=detection.circles[1].circularity;
    const zCirc=detection.circles[2].circularity;
    const geometryScore=
      clamp(1-yError/0.70,0,1)*0.42+
      clamp(1-radiusSpread/0.34,0,1)*0.24+
      clamp((yCirc-0.20)/0.55,0,1)*0.12+
      clamp((zCirc-0.35)/0.55,0,1)*0.22;
    const confidence=clamp(geometryScore*0.55+clamp((maskScore-0.18)/0.52,0,1)*0.45,0,1);
    ...
    // X is naturally the most occluded slot. A useful real-game registration
    // should not be rejected because X is a weak contour when Y/Z and the
    // permanent plaque geometry agree strongly.
    const locked=
      yError<=0.62 &&
      radiusSpread<=0.30 &&
      maskScore>=0.34 &&
    ...
    const readingText=`X=${readings.X.emoji.replace("emoji","E")}, Y=${readings.Y.emoji.replace("emoji","E")}, Z=${readings.Z.emoji.replace("emoji","E")}`;
    if(locked){
      setStatus(`INVENTORY LOCKED. Permanent geometry ${pct(maskScore)}; Y/Z rim confirmation passed. Recognition: ${readingText}.`,"good");
    }else{
      setStatus(`Candidate not fully locked. Geometry error ${yError.toFixed(2)}, radius spread ${pct(radiusSpread)}, permanent geometry ${pct(maskScore)}. Recognition still ran for review: ${readingText}.`,"bad");
    }
  }catch(err){
    console.error(err);
...
function captureCamera(){
  ...
  // IMPORTANT: capture exactly the image visible inside the short viewfinder.
  // CSS uses object-fit: cover, so the raw camera frame contains more vertical
  // content than the user can see. Reproduce that same center crop here.
  ...
  img.onload=()=>{
    ...
    setStatus("Captured exactly the visible viewfinder crop. Analyzing…","working");
    analyze();
  };
