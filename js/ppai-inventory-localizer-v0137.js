...
const REF=window.PPAI_INVENTORY_REFERENCE_V0137;
...
function markCvReady(){
  ...
  if(cvReady){
    opencvStatus.textContent="OpenCV ready — v0.13.7 geometry-first localizer available.";
...
async function ensureEmojiReferences(){
  ...
  for(let id=1;id<=18;id++){
    ...
    const img=await loadImage(`images/${emoji}.png?v=0137`);
    const sw=img.naturalWidth||img.width,sh=img.naturalHeight||img.height;
...
function classifyCrop(canvas,slot,refs){
  ...
  const ranked=candidates.map(r=>{
    ...
    return {emoji:r.emoji,id:r.id,score:color*0.16+shape*0.56+structure*0.28};
  }).sort((a,b)=>b.score-a.score);
  const best=ranked[0],second=ranked[1];
  const margin=Math.max(0,best.score-second.score);
  const familyMargin=Math.max(0,families[0].score-(families[1]?.score||0));
  const confidence=clamp(0.18+margin*4.4+familyMargin*1.5,0,1);
...
async function buildTemplateEdgePoints(){
  ...
  const [img,maskImg]=await Promise.all([
    loadImage("images/inventory_reference_v0137.png?v=0137"),
    loadImage("images/inventory_reference_mask_v0137.png?v=0137")
  ]);
