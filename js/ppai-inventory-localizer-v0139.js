...
const REF=window.PPAI_INVENTORY_REFERENCE_V0139;
...
function markCvReady(){
  ...
  if(cvReady){
    opencvStatus.textContent="OpenCV ready — v0.13.9 geometry-first localizer available.";
...
async function ensureEmojiReferences(){
  ...
  for(let id=1;id<=18;id++){
    ...
    const img=await loadImage(`images/${emoji}.png?v=0139`);
    const sw=img.naturalWidth||img.width,sh=img.naturalHeight||img.height;
...
async function buildTemplateEdgePoints(){
  ...
  const [img,maskImg]=await Promise.all([
    loadImage("images/inventory_reference_v0139.png?v=0139"),
    loadImage("images/inventory_reference_mask_v0139.png?v=0139")
  ]);
